...
struct keyval_st {
  ...
  explicit keyval_st(size_t num_, size_t ops)
  : key{num_}
  , val{num_}
  , num{num_}
  , ops(ops)
...
static size_t execute_hybrid(const client_options &opt, memcached_st &memc, const keyval_st &kv) {
  random64 rnd{};
  for (auto i= 0u; i < kv.ops; ++i) {
    auto r = rnd(0, 2);
    if(r) {
      auto r = rnd(0, kv.num);
      memcached_set(&memc, kv.key.chr[r], kv.key.len[r], kv.val.chr[r], kv.val.len[r], 0, 0);
    } else {
      auto r = rnd(0, kv.num);
      memcached_return_t rc;
      free(memcached_get(&memc, kv.key.chr[r], kv.key.len[r], nullptr, nullptr, &rc));
...
private:
  ...
  void execute() {
    ...
    } else if (!strcmp("get", opt.argof("test"))) {
      count = execute_get(opt, memc, kv);
    } else{
      if (strcmp("hybrid", opt.argof("test"))) {
        if (!opt.isset("quiet")) {
          std::cerr << "Unknown --test: '" << opt.argof("test") << "'.\n";
        }
      }
      count = execute_hybrid(opt, memc, kv);
    }
  }
};
